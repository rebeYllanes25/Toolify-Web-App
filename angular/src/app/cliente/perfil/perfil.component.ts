...
export class PerfilComponent implements OnInit {
  ...
  cancelarVenta(idVenta: number, index: number): void {
    AlertService.confirm(
      'Esta acción cancelará la venta y no se puede deshacer.',
      '¿Cancelar venta?',
      'Sí, cancelar',
      'No, volver'
    ).then((confirmed) => {
      ...
      this.compraService.cancelarVenta(idVenta).subscribe({
        next: (resultado) => {
          if (resultado.valor) {
            AlertService.success(resultado.mensaje);
            setTimeout(() => window.location.reload(), 1500);
          } else {
            AlertService.error(resultado.mensaje);
          }
        },
        error: (err) => {
          console.error(err);
          AlertService.error('Ocurrió un error al cancelar la venta.');
        },
      });
    });
  }
